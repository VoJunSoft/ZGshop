...
const newProductForm = (props) => {
  ...
  const uploadImage =  async (img) => {
    // if(image === props.productInfo.img)
    //     return  props.productInfo.img
        const uploadUri = img;
  ...
  return(
        ...
        <TextInput
           ...
           // autoFocus
         />
         <Text style={{paddingLeft:7, color: productInfo.productName.length < 5  ? 'red': 'green'}}>{productInfo.productName.length}/25</Text>
        ...
        <Text style={CSS.loading}>جاري التحميل</Text>
        </Overlay>
    </ScrollView>
    </>
    )
